import random
import logging
from manager.user_word_level_manager import UserWordLevelManager
...
class PracticeManager:
    def __init__(self, db_manager=None):
        self.user_word_level_manager = UserWordLevelManager(db_manager)
        self.word_manager = WordManager(db_manager)
        self.vocabulary_manager = VocabularyManager(db_manager)
        self.practice_session_manager = PracticeSessionManager(db_manager)
    ...
    def get_next_word(self, user_id):
        ...
        try:
            ...
            words_with_levels = self.user_word_level_manager.get_user_words_with_levels(user_id)
            ...
            word_details = self.word_manager.get_word_by_id(selected_word['word_id'])
            ...
            choices = self.generate_choices(word_details, level, user_id)
            
            # Format the response
            word_response = {
                'word_id': str(word_details['id']),
    ...
    def generate_choices(self, correct_word, level, user_id):
    ...
    def get_group_name_for_word(self, word_id):
        """Get the group name for a word"""
        try:
            word = self.word_manager.get_word_by_id(word_id)
    ...
    def submit_answer(self, user_id, word_id, selected_choice_index, correct_choice_index, time_taken, session_id=None):
        ...
        try:
            ...
            current_level_record = self.user_word_level_manager.get_user_word_level(user_id, word_id)
            current_level = current_level_record['level']
            
            # Update user's level for this word
            new_level = self.user_word_level_manager.update_user_word_level(user_id, word_id, is_correct)
            
            # Record progress
            if session_id:
                progress_record = self.user_progress_manager.record_progress(
                    user_id, 
                    word_id, 
                    session_id, 
